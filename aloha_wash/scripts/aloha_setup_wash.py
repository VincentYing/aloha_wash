...
def main():
    try:
        ...
        input("==== Press `Enter` to wash ...")
        frArm.go_to_joint_state((0, 52, 36, 0, 0, 0))
        rospy.sleep(2) # Let arm motion catch up
        frArm.go_to_joint_state((0, 54, 65, 0, 0, 0))
        frArm.go_to_joint_state((0, 24, 81, 0, 0, 0))
        rospy.sleep(2) # Let arm motion catch up
        frArm.go_to_joint_state((0, 14, 71, 0, 0, 0))
        frArm.go_to_start_state()

        res = input("==== Press `Enter` to attach sweep tool to rr arm ...")
        rrGripper.open_gripper((0.02, 0.02))
        rrArm.go_to_joint_state((26, 138, 123, -77, 0, 21))
        rrArm.attach_box()

        input("==== Press `Enter` to sweep ...")
        rrArm.go_to_joint_state((0, 52, 36, 0, 0, 0))
        rospy.sleep(2) # Let arm motion catch up
        rrArm.go_to_joint_state((0, 54, 65, 0, 0, 0))
        rrArm.go_to_joint_state((0, 24, 81, 0, 0, 0))
        rospy.sleep(2) # Let arm motion catch up
        rrArm.go_to_joint_state((0, 14, 71, 0, 0, 0))
        rrArm.go_to_start_state()

        res = input("==== Press `Enter` to attach spray tool to fl arm ...")
        flGripper.open_gripper((0.02, 0.02))
        flArm.go_to_joint_state((-99, 77, 69, -84, 0, 84))
        flArm.attach_box()

        input("==== Press `Enter` to spray ...")
        flArm.go_to_joint_state((-90, 65, 75, 0, 0, 0))
        rospy.sleep(2) # Let arm motion catch up
        flArm.go_to_joint_state((-135, 65, 75, 0, 0, 0))
        flArm.go_to_joint_state((-134, 63, 64, 0, 0, 0))
        rospy.sleep(2) # Let arm motion catch up
        flArm.go_to_joint_state((-90, 63, 59, 0, 0, 0))
        flArm.go_to_start_state()

        res = input("==== Press `Enter` to attach dry tool to lr arm ...")
        lrGripper.open_gripper((0.02, 0.02))
        lrArm.go_to_joint_state((-27, 130, 107, -61, 0, -32))
        lrArm.attach_box()

        input("==== Press `Enter` to dry ...")
        lrArm.go_to_joint_state((-46, 102, 131, 0, 0, 0))
        rospy.sleep(2) # Let arm motion catch up
        lrArm.go_to_joint_state((-96, 60, 90, 0, 0, 0))
        lrArm.go_to_joint_state((-96, 60, 50, 0, 0, 0))
        rospy.sleep(2) # Let arm motion catch up
        lrArm.go_to_joint_state((-46, 102, 102, 0, 0, 0))
